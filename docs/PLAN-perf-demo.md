...
|---|---|---|---|
| **R0 Spec** ✅ | task-set family `kind="tasks"`: per task `C,T,D`, policy `RM`/`DM`/`EDF`; auto-detected from `[[task]]`. `examples/models/tasks.model.toml`. | reuse TOML parser/family detect | — |
| **R1 Native analysis (M1-analog)** ✅ | `rt.rs`: utilization-bound test `O(n)` + exact **RTA** fixed point `Rᵢ = Cᵢ + Σⱼ∈hp ⌈Rᵢ/Tⱼ⌉Cⱼ` (RM/DM), EDF via `U≤1`; `lift model check` → schedulable verdict + WCRT table. The teaching case (U>bound yet RTA-schedulable) ships. | new `rt.rs`; reuse report/CLI | `rta_exact_beats_the_bound`, overload teeth, **Liu–Layland property** (util-pass ⇒ RTA-schedulable, 3000 sets); ci.sh schedulability + teeth |
| **R2 Kernel proof (Kani)** ✅ | the RTA per-term kernel `term(r,cj,tj)=⌈r/tj⌉·cj` (used by `response_time`) proved **overflow-free** (`div_ceil_safe`, incl. `⌈a/b⌉≤a`) and **monotone in `r`** (`term_monotone`) — monotonicity ⇒ the iteration converges to the LEAST fixed point = the true WCRT, so the analyzer is sound. `verify-kani.sh` (deep tier). | reuse the `fire_no_underflow` Kani pattern | KANI GREEN (3 harnesses). *Aeneas/Lean deductive version optional.* |
| **R3 Empirical cross-check** ✅ | discrete-event preemptive RM scheduler from the critical instant; `rta_matches_simulation` asserts observed WCRT == exact RTA | test-only simulator | sim-vs-RTA teeth (a wrong RTA goes red) |
| **R4 The intersection demo ★** ✅ | one task/load model → **hard** schedulability (RTA, a step) **and** **soft** deadline-miss prob (Monte-Carlo, exec∈[0.5C,C], a sigmoid) vs load `--scale`; `lift model check/simulate --scale`, `scripts/tasks-sweep.sh`. Hard boundary scale≈1.3 yet soft miss only 0.13 there → **conservative**. | reuse R1 RTA + a stochastic RM sim | conservatism check (`--check`): soft miss low at the hard boundary; in ci.sh |
| **R5 Narrative** ✅ | `examples/models/tasks.recipe.md`: "provably safe (hard) vs probably safe (soft) — pick your margin" | reuse recipe writer | — |
