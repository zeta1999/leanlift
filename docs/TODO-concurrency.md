...
- [ ] A2 define `wp e {Φ}` over `λ-conc`; prove the adequacy theorem (closes the model-code gap for this lane)
      ...
      - [ ] A2.2 `wp` fixpoint over `λ-conc` (relational `prim_step`, `fork`/`efs`, `state_interp`) via BI `Lib/Fixpoint`; `wp_unfold`; contractivity. Template: Example3's `wp_F`.
      - [ ] A2.3 lifting lemmas (one per `Head` rule: pure beta/if/binop, alloc/load/store, CAS/FAA, fork) + bind via `fill_app`.
