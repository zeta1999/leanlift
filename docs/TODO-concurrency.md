...
- [~] B2 weak-memory assertions — **started**: `LeanliftIris/PhaseB/Logic.lean` — `Monotone` view-assertions (the subjective props of iRC11), **`ra_transfer`** (the synchronizes-with rule: any monotone assertion the releaser established transfers to an acquire-reader, on top of `release_acquire_hb`), the `seen l t` assertion + `seen_mono`/`seen_after_store`, and `mp_via_logic` (message passing re-derived *in the logic*). Remaining B2: a full rel/acq points-to (`l ↦` as a view-indexed assertion), objective/subjective split, fences, the StoreLoad/`seq_cst` edge.
